...
Run a convergence study on $P(30)$, the phosphorus concentration thirty years in. Build a reference solution at a very fine step size, then test a sequence of successively-halved, coarser step sizes against it. Here is the starting point:
...
Step sizes that divide 30 evenly keep that indexing simple — $\Delta t = 2, 1, 0.5, 0.25, 0.125, 0.0625$ all do.
...
::: {.callout-tip}
## Why $P(30)$, and not the value at the end of the run?

A quantity of interest is only useful for a convergence study if it actually *depends* on the step size. By the end of a 300-year run this lake has settled into its long-run state, and every stable step size lands on the same number to six or more digits — there would be nothing left to measure. Thirty years in, the lake is still changing, so the step size still matters.

This is the same reason the lecture example used a peak concentration rather than an end state: pick something the discretization can actually get wrong.
:::
